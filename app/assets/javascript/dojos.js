    ...
    $(document)
        ...
        .on("click", "#newStudent", function(){
            ...
            $.get($(this).attr("href"), function(res){
                ...
                $("#updateStudentDojo").html(dojoHTML)
            })
        ...
        .on("submit", "#newDojoForm", function(e){
            ...
            $.post($(this).attr('action'), $(this).serialize(), function(res){
                ...
                html    += "<a action-type='edit_dojo' href='/dojos/" + res.dojo.id + "/edit'>Edit</a>"
                html    += "<a action-type='delete_dojo' href='/dojos/" + res.dojo.id + "'>Destroy</a>"
                html    += "</td>"
                html    += "</tr>" 
                $("tbody").append(html)
        ...
        .on("submit", "#updateDojoForm", function(e){
            ...
            $.post($(this).attr("action"), $(this).serialize(), function(res){
                ...
                if ($("#updateDojoForm :input[name=form_location]").val() == "index"){
                    ...
                    dojoHTML    += "<td>"
                    dojoHTML    += "<a href='/dojos/" + res.id + "'>Show</a> "
                    dojoHTML    += "<a action-type='edit_dojo' href='/dojos/" + res.id + "/edit'>Edit</a>"
                    dojoHTML    += "<a action-type='delete_dojo' href='/dojos/" + res.id + "'>Destroy</a>"
                    dojoHTML    += "</td>"
        ...
        .on("submit", "#updateStudentForm", function(e){
            ...
            $.post($(this).attr("action"), $(this).serialize(), function(res){
                let studentHTML  = "<td>"+ res.first_name +"</td>"
                studentHTML     += "<td>"+ res.last_name +"</td>"
                studentHTML     += "<td>"+ res.email +"</td>"
                studentHTML     += "<td>"
                studentHTML     += "<a href='/students/"+ res.id +"'> Show</a>"
                studentHTML     += "<a action-type='edit_student' href='/students/" + res.id + "/edit'> Edit</a>"
                studentHTML     += "</td>"
                $("#student" + res.id).html(studentHTML)
            })
        ...
        .on("submit", "#createStudentForm", function(e){
            ...
            $.post($(this).attr("action"), $(this).serialize(), function(res){
                ...
                if(res.student.dojo_id == res.current_dojo){
                    ...
                    studentHTML     += "<a href='/students/" + res.student.id + "/edit'>Edit</a>"
                    studentHTML     += "</td>"
